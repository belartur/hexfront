...
rust/src/            moduły gry (podział i nazwy ustalamy przy implementacji)
```
...
Testy logiki mają działać bez okna (bez inicjalizacji grafiki). Zakres: reguły z [rules.md](rules.md) (ruch, produkcja, walka, działka, wieże, eliminacja, AI) oraz format plansz — odczyt tych samych plików z `maps/` musi dawać ten sam stan początkowy w każdej implementacji, co wynika z formatu ([specification_of_map_format.md](specification_of_map_format.md)), a nie z jednej implementacji.
...
* cache nieruchomego terenu i reguły jego unieważniania — punktem odniesienia może być sekcja „Renderowanie” w [specification_python.md](specification_python.md), ale rozwiązanie może być inne;
